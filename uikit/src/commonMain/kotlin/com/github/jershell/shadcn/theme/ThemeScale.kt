...
/**
 * A base color scale of the theme builder: 11 shades from `50` to `950`, indexed
 * 0..10 (`0` = 50, `5` = 500, `10` = 950), exactly like the Tailwind palette.
 */
public class ThemeScale(
    val name: String,
    private val hexes: List<String>,
) {
    init {
        check(hexes.size == 11) { "ThemeScale '$name' must have 11 shades" }
    }

    /** Shade by index: `0` = 50, `1` = 100, ..., `10` = 950. */
    operator fun get(index: Int): Color = Color(hexes[index].toLong(16) or 0xFF000000)

    companion object {
        fun parse(name: String, vararg hexes: String) = ThemeScale(name, hexes.toList())
    }
}

/**
 * Base color scales of the theme builder, matching the color pickers of the
 * shadcn/ui create page (Tailwind palette). The neutral grays come first — they
 * are used for the `Base Color` picker; `"neutral"` itself resolves to the
 * generated default theme (the Figma tokens are the source of truth for it).
 */
val themeScales: List<ThemeScale> = listOf(
    ThemeScale.parse(
        "neutral",
...
internal fun themeScaleOrNull(name: String): ThemeScale? =
    themeScales.firstOrNull { it.name == name }